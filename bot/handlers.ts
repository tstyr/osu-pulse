...
async function handleOsu(interaction: ChatInputCommandInteraction) {
  ...
  if (subcommand === "unlink") {
    ...
    await interaction.reply({ content: deleted ? "登録と追跡データを削除しました。" : "登録済みアカウントはありません。", flags: MessageFlags.Ephemeral });
    return;
  }

  if (subcommand === "daily") {
    const enabled = interaction.options.getBoolean("enabled", true);
    const account = await setDailyDm(interaction.user.id, enabled);
